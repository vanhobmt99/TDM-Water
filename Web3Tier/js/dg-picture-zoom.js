...
if(!window.DG) {
    window.DG = {};
    var dE = document.documentElement, gE = document.getElementsByTagName('body')[0], viewportX = window.innerWidth || dE.clientWidth || gE.clientWidth, viewportY = window.innerHeight || dE.clientHeight || gE.clientHeight, undHierNochEineSinnfreieVariable = 0;
    // uncomment the following line to see
    // your viewport size
    //alert (viewportX+" x " + viewportY);
    var mitteX = viewportX / 2, mitteY = viewportY / 2;
...
DG.PictureZoom = new Class({
	...
	_zoom : function(id, direction) {
        ...
        if(direction == 'in'){
            ...
            var to = {
                'left' : mitteX - (obj.large.width /2),
                'top' : Math.max(8, Math.round(coords.small.top - (obj.large.height /2) + coords.small.height/2) ),
